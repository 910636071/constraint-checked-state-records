...
production code. It is a small artifact paper / technical note, not a broad
empirical claim.
\end{abstract}
...
In this paper, clean-room means that the artifact does not use real user data,
production code, private terminology, previous draft content, or external
document content. The case bank is generated as synthetic structured data
...
that isolates one checked-record interface for inspection.
...
The artifact is available in the public
\href{https://github.com/910636071/constraint-checked-state-records}{repository}.
It is released under the MIT License and includes the synthetic case bank,
pipeline implementation, pilot scripts, tests, term scan, paper notes, and
aggregate CSV summary. A versioned review should cite the repository URL
together with the specific Git commit or release tag used for inspection. The
pilot records and summary can be regenerated with the commands in the previous
section.
...
is to provide a small reproducible object for inspection, not to establish
